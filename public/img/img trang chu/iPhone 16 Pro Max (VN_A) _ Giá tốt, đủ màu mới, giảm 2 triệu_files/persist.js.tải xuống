...
var ADM_AdsTracking = {
    ...
    check: function (id, val) {
		function getCookie(e){var i=e+"=",a=window.unescape(document.cookie).split(";");for(let t=0;t<a.length;t++){let e=a[t];for(;" "==e.charAt(0);)e=e.substring(1);if(0==e.indexOf(i))return e.substring(i.length,e.length)}return""};
		var refer=document.referrer+'';
		var uidCk=getCookie('__uid');
       	id=id+'';
		val=val+'';
		var chkid=false;
		/*if(id=='__uid'){
		...
		if(id=='__uid'){
			...
			try{
				if(uidCk!="" && chkid!=uidCk){				
					var isValidCk=/([^\d]+)/.test(uidCk);
					if(!isValidCk){
						chkid=uidCk;
						ADM_AdsTracking.store.set(id , uidCk);
					}
				}
			}catch(e){}

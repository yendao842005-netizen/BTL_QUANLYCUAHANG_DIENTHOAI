$(document).ready(function () {
  ...
  function fetchData(page) {
    ...
    $("#categoriesTable tbody").html(
      '<tr><td colspan="7" class="text-center py-4">Đang tải dữ liệu... <i class="fas fa-spinner fa-spin"></i></td></tr>'
    );
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (sortFilter) {
          ...
          categories.sort((a, b) =>
            a.MaDM.localeCompare(b.MaDM, undefined, { numeric: true })
          );
        }
  ...
  function updateStats() {
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#avgRevenue").text(
          formatCurrency(avg).replace("₫", "").trim() + " ₫"
        );
        ...
        $("#lastUpdated").text(
          `Cập nhật: ${now.toLocaleDateString("vi-VN", options)}`
        );
      },
      error: function (err) {
  ...
  function showNoResult() {
    $("#categoriesTable tbody").html(
      '<tr><td colspan="7" class="text-center py-4">Không tìm thấy danh mục nào!</td></tr>'
    );
    $("#pagination").empty();
                ...
                <tr data-category="${catJson}">
                    ...
                    <td>
                        <div class="category-info">
                            ...
                            <div class="category-description text-muted small">${
                              cat.MoTa || ""
                            }</div>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="action-buttons">
                            <button class="action-btn view-btn" title="Xem chi tiết" onclick="viewCategoryDetail('${
                              cat.MaDM
                            }')">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button class="action-btn edit-btn" title="Sửa" onclick="editCategory('${
                              cat.MaDM
                            }')">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="action-btn delete-btn" title="Xóa" onclick="deleteCategory('${
                              cat.MaDM
                            }')">
  ...
  window.saveCategory = function () {
    ...
    // Chỉ bắt buộc nhập Tên danh mục
    if (!data.TenDanhMuc) {
  ...
  window.editCategory = function (maDM) {
    ...
    $.ajax({
      ...
      success: function (cat) {
        ...
        // Mặc định select
        $('#editCategoryForm select[name="TrangThai"]').val(
          cat.TrangThai || "active"
        );
        $('#editCategoryForm select[name="Icon"]').val(
          cat.Icon || "mobile-alt"
        );
  ...
  window.deleteCategory = function (maDM) {
    ...
    if (confirm(`Bạn có chắc muốn xóa danh mục ${maDM}?`)) {
      $.ajax({
        ...
        error: function (err) {
          alert(
            "Lỗi khi xóa: " + (err.responseJSON?.message || "Không xác định")
          );
        },
      });
    }
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="detail-section">
                                <h5><i class="fas fa-info-circle"></i> Thông tin chung</h5>
                                <table class="detail-table">
                                    <tr><td><strong>Mô tả:</strong></td><td>${
                                      cat.MoTa || "Chưa có mô tả"
                                    }</td></tr>
                        ...
                        <div class="col-md-6">
                            <div class="detail-section">
                                ...
                                <table class="detail-table">
                                    ...
                                    <tr><td><strong>Doanh thu:</strong></td><td><span class="text-success fw-bold">${formatCurrency(
                                      realRevenue
                                    )}</span></td></tr>
                                </table>
                            </div>
  ...
  function renderProductList(products) {
    ...
    let html =
      '<table class="table table-sm table-bordered mt-2"><thead class="table-light"><tr><th>Mã SP</th><th>Tên sản phẩm</th><th class="text-end">Giá bán</th><th class="text-center">Tồn</th></tr></thead><tbody>';
    products.forEach((p) => {
      html += `<tr><td><small>${p.MaSP}</small></td><td>${
        p.TenSanPham
      }</td><td class="text-end text-primary">${formatCurrency(
        p.GiaBan
      )}</td><td class="text-center">${p.SoLuongTon}</td></tr>`;
    });
    html += "</tbody></table>";
    return html;
  }
  ...
  window.exportCategories = async function () {
    if (!confirm("Xuất danh sách danh mục ra file Excel?")) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            a.href = url;
            a.download = `DanhSach_DanhMuc.xlsx`;
            document.body.appendChild(a);
            a.click();
            a.remove();
        } else {
            alert("Lỗi xuất file! Mã lỗi: " + response.status);
        }
    } catch (e) {
        console.error(e);
  ...
  function loadCategoryOptions() {
    $.ajax({
      ...
      success: function (data) {
        ...
        data.forEach((cat) => {
          select.append(
            `<option value="${cat.MaDM}">${cat.TenDanhMuc}</option>`
          );
        });
      },
      error: function (err) {

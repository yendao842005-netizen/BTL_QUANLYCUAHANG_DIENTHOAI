$(document).ready(function () {
        ...
        orders.forEach(order => {
            ...
            html += `
                <tr>
                    ...
                    <td><span class="text-muted text-truncate" style="max-width: 150px; display: inline-block;">${order.GhiChu || ''}</span></td>
                    <td>
                        <div class="action-buttons">
                            <button class="action-btn view-btn" title="Xem chi tiết" onclick="viewOrderDetail('${order.MaHD}')">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button class="action-btn edit-btn" title="Cập nhật trạng thái" onclick="openUpdateStatusModal('${order.MaHD}', '${order.TrangThai}')">
    ...
    window.viewOrderDetail = function (maHD) {
        ...
        $.when(
            ...
            const hinhThucTT = order.PhuongThucThanhToan || order.HinhThucTT || 'cash';
            ...
            if (items && items.length > 0) {
                items.forEach((item, index) => {
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="fw-bold">${item.TenSanPham || 'Mã SP: ' + item.MaSP}</div>
                        </td>
            ...
            <div class="order-detail">
                <div class="order-header mb-4 pb-3 border-bottom">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h3 class="m-0 text-primary">Đơn hàng #${order.MaHD}</h3>
                        <span class="status ${order.TrangThai}">
                            ${getStatusText(order.TrangThai)}
                        </span>
                    </div>
                    <div class="text-muted small">
                        <i class="far fa-clock"></i> Ngày đặt: ${formatDate(order.NgayLap)}
                    </div>
                </div>
        ...
        `;
            ...
            alert('Không thể tải dữ liệu chi tiết đơn hàng. Vui lòng kiểm tra console!');
        });
    };
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                employees.forEach(nv => {
                    ...
                    employeeSelect.append(`<option value="${nv.MaNV}">${nv.MaNV} - ${nv.HoTen}</option>`);
                });
            },
            error: function (err) { console.error("Lỗi tải nhân viên:", err); }
        });
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                customers.forEach(kh => {
                    ...
                    $(`<option value="${kh.MaKH}">${kh.MaKH} - ${kh.HoTen}</option>`).insertBefore(customerSelect.find('option[value="new"]'));
                });
            },
            error: function (err) { console.error("Lỗi tải khách hàng:", err); }
        });
    }
    ...
    $('#productSearchInput').on('input', function () {
        ...
        $results.show();
        $results.html('<div class="p-2 text-muted text-center"><i class="fas fa-spinner fa-spin"></i> Đang tìm...</div>');

        productSearchTimeout = setTimeout(function () {
            // --- SỬA LẠI URL THEO YÊU CẦU CỦA BẠN ---
            // Viết tham số trực tiếp vào URL: ?TenSanPham=...
            const searchUrl = `${PRODUCT_API_URL}/SearchAdvanced?ten=${encodeURIComponent(keyword)}`;
                ...
                success: function (response) {
                    ...
                    if (products.length > 0) {
                        products.forEach(p => {
                            ...
                            const itemHtml = `
                                <div class="search-result-item" onclick="addProductToOrder('${p.MaSP}', '${p.TenSanPham}', ${p.GiaBan})">
                                    <div class="d-flex justify-content-between">
                                        <span class="fw-bold">${p.TenSanPham}</span>
                                        <span class="text-primary">${price}</span>
                                    </div>
                                    <div class="small text-muted">Mã: ${p.MaSP} | Tồn: ${p.SoLuongTon || 0}</div>
                                </div>
                            `;
                            $results.append(itemHtml);
                        });
                    } else {
                        $results.html('<div class="p-2 text-center text-muted">Không tìm thấy sản phẩm nào</div>');
                    }
                },
                error: function (err) {
                    console.error("Lỗi tìm kiếm:", err);
                    $results.html('<div class="p-2 text-center text-danger">Lỗi kết nối server!</div>');
                }
            });
        }, 300);
    });
                ...
                if (products && products.length > 0) {
                    const html = products.slice(0, 5).map(p => `
                        <div class="search-result-item" onclick="addProductToOrder('${p.MaSP}', '${p.TenSanPham}', ${p.GiaBan})">
                            ...
                            <div class="result-price">${formatCurrency(p.GiaBan)}</div>
                        </div>
                    `).join('');
                    $('#productSearchResults').html(html);
                } else {
                    $('#productSearchResults').html('<div class="p-2 text-muted">Không tìm thấy sản phẩm</div>');
                }
            }
        });
    }
        ...
        orderItems.forEach((item, index) => {
            ...
            tbody.append(`
                <tr>
                    ...
                    <td>
                        <input type="number" class="form-control form-control-sm" style="width: 60px" value="${item.quantity}" min="1" onchange="updateItemQty(${index}, this.value)">
                    </td>
    ...
    window.createOrder = function () {
        ...
        const btnCreate = document.querySelector('#createOrderForm button[onclick="createOrder()"]');
        const originalText = btnCreate.innerHTML;
        btnCreate.disabled = true;
        btnCreate.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Đang xử lý...';
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        Promise.all(promises)
                            ...
                            .catch(err => {
                                console.error(err);
                                alert(`Đơn hàng ${createdMaHD} đã tạo nhưng lỗi khi lưu sản phẩm!`);
                            })
                            .finally(() => {
                                // Khôi phục nút bấm
                    ...
                    error: function (err) {
                        ...
                        if (err.responseText && err.responseText.includes('Duplicate')) {
                            alert('Hệ thống vừa phát sinh đơn hàng mới trùng mã. Vui lòng thử lại!');
                        } else {
                            alert('Lỗi tạo đơn hàng: ' + (err.responseJSON?.message || 'Lỗi server'));
                        }
                    }
                });
            },
    ...
    window.deleteOrder = function (maHD) {
        if (confirm(`Bạn có chắc chắn muốn xóa đơn hàng ${maHD}? Hành động này không thể hoàn tác.`)) {
            $.ajax({
                url: `${API_URL}/${maHD}`,
                method: 'DELETE',
                success: function () {
                ...
                error: function (err) {
                    alert('Không thể xóa đơn hàng: ' + (err.responseJSON?.message || 'Lỗi server'));
                }
            });
        }
    }
    ...
    window.exportToExcel = function () {
        // Mặc dù api.js chưa có route này, nhưng đây là cách viết chuẩn
        // Bạn nên thêm route router.get("/HoaDons/Export/Excel", ...) vào backend
        if (confirm("Xuất danh sách đơn hàng ra Excel?")) {
            // Tạm thời dùng Dashboard export nếu chưa có API riêng
    ...
    function getStatusText(s) {
        const map = { 'ChoXuLy': 'Chờ xử lý', 'HoanThanh': 'Hoàn thành', 'DaHuy': 'Đã hủy', 'DangGiao': 'Đang giao' };
        return map[s] || s;
    }
    function getStatusClass(s) {
        const map = { 'ChoXuLy': 'pending', 'HoanThanh': 'completed', 'DaHuy': 'cancelled', 'DangGiao': 'shipping' };
        return map[s] || 'pending';
    }
    function getPaymentText(p) {
        const map = { 'TienMat': 'Tiền mặt', 'ChuyenKhoan': 'Chuyển khoản', 'The': 'Thẻ' };
        return map[p] || p;
    }
    ...
    window.processAllPendingOrders = function () {
        alert("Tính năng này cần Backend hỗ trợ xử lý hàng loạt (Batch Processing). Hiện tại chưa khả dụng.");
    }

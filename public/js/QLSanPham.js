$(document).ready(function () {
  ...
  function fetchData(page) {
    ...
    let finalSearchName =
      categoryVal && categoryVal !== "" ? categoryText : keyword;
    ...
    $("#productsTable tbody").html(
      '<tr><td colspan="8" class="text-center py-4">Đang tải... <i class="fas fa-spinner fa-spin"></i></td></tr>'
    );
  ...
  function renderTable(products) {
    ...
    products.forEach((sp) => {
      // Format Tiền & Ngày
      let giaBan = new Intl.NumberFormat("vi-VN", {
        style: "currency",
        currency: "VND",
      }).format(sp.GiaBan);
      let ngayNhap = sp.NgayNhap
        ? new Date(sp.NgayNhap).toLocaleDateString("vi-VN")
        : "";
                ...
                <tr data-product="${spJson}">
                    ...
                    <td>
                        <div class="product-info">
                            ...
                            <div class="product-spec text-muted small">${
                              sp.TenNCC || sp.MaNCC
                            }</div>
                        </div>
                    </td>
                ...
                <div class="row" style="display:flex; gap:20px;">
                    <div class="col-md-6" style="flex:1">
                         <p><strong>Danh mục:</strong> ${
                           product.TenDM || product.MaDM
                         }</p>
                         <p><strong>Nhà cung cấp:</strong> ${
                           product.TenNCC || product.MaNCC
                         }</p>
                         <p><strong>Ngày nhập:</strong> ${
                           product.NgayNhap
                             ? new Date(product.NgayNhap).toLocaleDateString(
                                 "vi-VN"
                               )
                             : ""
                         }</p>
                    ...
                    <div class="col-md-6" style="flex:1">
                         <p><strong>Giá bán:</strong> ${formatCurrency(
                           product.GiaBan
                         )}</p>
                         <p><strong>Trạng thái:</strong> ${
                           product.SoLuongTon > 20 ? "Còn hàng" : "Sắp hết"
                         }</p>
                         <p><strong>Hình ảnh:</strong> ${
                           product.HinhAnh || "Chưa có"
                         }</p>
                    </div>
                </div>
  ...
  window.editProduct = function (maSP) {
    ...
    $.ajax({
      ...
      success: function (product) {
        ...
        if (product.NgayNhap) {
          try {
            let dateVal = new Date(product.NgayNhap)
              .toISOString()
              .split("T")[0];
  ...
  window.saveProduct = function () {
    ...
    // Validate cơ bản: BỎ kiểm tra data.MaSP
    if (!data.TenSanPham || !data.MaDM || !data.MaNCC) {
        alert("Vui lòng nhập tên sản phẩm, danh mục và nhà cung cấp!");
        return;
      }
    ...
    else data.SoLuongTon = 0; // Mặc định là 0 nếu không nhập
    // =======================
    ...
    $.ajax({
      ...
      error: function (err) {
        ...
        let msg =
          ...
          "Mã SP có thể đã tồn tại hoặc lỗi server";
 ...
 // --- CẬP NHẬT HÀM XUẤT EXCEL (Sửa lỗi thiếu Token) ---
window.exportToExcel = async function () {
  if (!confirm("Xác nhận xuất danh sách sản phẩm ra Excel?")) return;
  ...
  try {
      ...
      // Xử lý File Blob để tải xuống
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const dateStr = new Date().toISOString().slice(0, 10);
      a.download = `DanhSach_SanPham_${dateStr}.xlsx`;
        ...
        data.forEach((item) => {
          ...
          html += `
                        <tr>
                            ...
                            <td class="text-center">
                                <button class="btn btn-sm btn-outline-primary btn-category-detail" data-id="${item.MaDM}">
  ...
  window.filterByCategory = function (maDM) {
    // Gán giá trị vào select box
    $("#categoryFilter").val(maDM).trigger("change"); // Dùng .trigger('change') để đảm bảo sự kiện chạy
      ...
      success: function (suppliers) {
        ...
        suppliers.forEach((ncc) => {
          supplierSelects.append(
            `<option value="${ncc.MaNCC}">${ncc.TenNhaCungCap}</option>`
          );
        });
      },
      error: function () {
  ...
  // Nút Lưu và Cập nhật trong Modal
  ...
  // Các nút Đóng/Hủy Modal (Dùng chung class hoặc selector)
  $(".close-modal, .btn-outline").click(function () {
    // Tìm modal cha gần nhất để đóng
    let modalId = $(this).closest(".modal").attr("id");
    if (modalId) window.closeModal(modalId);
  });

  // Các nút động trong bảng (Dùng Event Delegation)
  // Vì bảng được vẽ lại sau khi tải trang, nên phải dùng $(document).on(...)
  ...
  // Nút "Chỉnh sửa" trong Modal Xem chi tiết
  $(document).on("click", "#viewProductModal .btn-primary", function () {
    window.editCurrentProduct();
  });
  // Nút "Chỉnh sửa" (Chuyển từ Xem -> Sửa)
  $("#btnEditCurrentProduct").click(function () {
    window.editCurrentProduct();
  });

  $(document).on('click', '.btn-category-detail', function() {
    let maDM = $(this).data('id');
    
    // Gọi hàm lọc (đã định nghĩa ở các bước trước)
    if (typeof window.filterByCategory === 'function') {
        window.filterByCategory(maDM);
    } else {
        // Dự phòng nếu chưa có hàm: Tự xử lý luôn
        $("#categoryFilter").val(maDM).trigger('change');
        $('html, body').animate({
            scrollTop: $(".table-container").first().offset().top - 100
        }, 500);
    }

...
export const NhanVienRepository = {
  ...
  update: async (MaNV, data) => {
    ...
    try {
      ...
      // 1. Tạo mảng các trường cần update
      const fields = [];
      const values = [];
      ...
      if (data.ChucVu !== undefined) {
        fields.push("ChucVu = ?");
        values.push(data.ChucVu);
      }
      if (data.LuongCoBan !== undefined) {
        fields.push("LuongCoBan = ?");
        values.push(data.LuongCoBan);
      }
      if (data.NgayVaoLam !== undefined) {
        fields.push("NgayVaoLam = ?");
        values.push(data.NgayVaoLam);
      }

      // Nếu không có trường nào để update thì return luôn
      if (fields.length === 0) {
        return await NhanVienRepository.getByMa(MaNV);
      }

      // Thêm MaNV vào cuối mảng values cho mệnh đề WHERE
      values.push(MaNV);

      // 2. Tạo câu SQL động
      const query = `UPDATE NhanVien SET ${fields.join(", ")} WHERE MaNV = ?`;
